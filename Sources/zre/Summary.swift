...
enum Summary {
    static func print(stats: Stats, options: Options) {
        ...
        per-frame latency ms  : median \(String(format: "%.2f", stats.median)), p95 \(String(format: "%.2f", stats.p95)), mean \(String(format: "%.2f", stats.mean)) (n=\(stats.latencyCount)) [hash + embed + store]
