...
Phases 3 and 4 are also **code complete on Intel**. Phase 3 (red-team + defense) has its machine-independent math (defense transforms, leakage scoring, retrieval retention, store round-trip) verified by numpy-only self-tests that pass here (`selftest_defense.py`, `selftest_retrieval.py`); the attack/defense *fidelity numbers* are M5-bound. Phase 4 (cross-variant benchmark, web visualizer, DEMO.md) is written, with the PCA exporter verified here. The net state: **the entire project is built and logic-tested on the Intel Mac; the one remaining job is a single Apple-Silicon (M5) session that captures every machine-dependent number** — driven by `MEASURE.md` and `scripts/run_all_measurements.sh`. The Intel toolchain ceiling (below) is why the model-dependent verifications also land on the M5 rather than here.
...
- [ ] Phase 4 — Benchmarks + visualizer + storytelling (code complete; cross-variant bench numbers + demo numbers pending the M5 run)
...
## Phase 4 — code complete, benchmark + demo numbers pending

- `bench_all.py` — times any set of exported `.mlpackage` image encoders across ANE/GPU/CPU/ALL and prints a markdown table with a machine label; `MEASURE.md` section F exports S0/S1/S2/B and runs it for the cross-variant table. Machine-dependent -> M5.
- Visualizer — `export_viz.py` (numpy PCA-2D of the stored vectors -> `viz/data.json`, verified machine-independently on a synthetic store) + `viz/index.html`, a self-contained three-panel web app (RAM wipe animation, growing concept map, attacker reconstruction clean vs defended). `attack_inversion.py --dump viz` writes the reconstruction PNGs. Serve with `python3 -m http.server`, open `/viz/`.
- `DEMO.md` — LinkedIn post + 30–60 s video script + interview talking points, with every number left as a bracketed placeholder until the M5 run fills it.

Measured numbers (Phase 4): **all TBD** (cross-variant sizes/latency are machine-dependent; the demo numbers are aggregates of the other phases' measurements). The code paths are complete; the PCA exporter and the table emitter are the only machine-independent pieces and are verified.
...
## Next action
All four phases are code complete and logic-tested on Intel. The only remaining work is the **single Apple-Silicon (M5) measurement session**: follow `MEASURE.md` A→F (one command, `bash scripts/run_all_measurements.sh`, then the interactive capture/label/red-team/visualizer steps). It produces every `TBD` number — Phase 0 bench, Phase 0/2 export cosine, Phase 1/2 latency + zero-retention proof, Phase 2 retrieval precision, the Phase 3 before/after attack-vs-retrieval table, and the Phase 4 cross-variant table. Then replace the `TBD`s in README / RESUME_BULLETS / this file (and the brackets in DEMO.md) with the measured values, check off the phases, commit, and push.

Watch items for the M5 run, in priority order: (1) the text-encoder export converting (argmax pooling) — `verify_text_coreml.py` is the gate; (2) capturing at native resolution being fast enough at 2 fps (use `--capture-max-long-edge` if a 5K/6K panel makes OCR/resize slow); (3) reporting the Phase 3 trade-off honestly — additive noise may not separate leakage from retrieval, and that is a valid measured result.
